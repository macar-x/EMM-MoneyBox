...
(UUID(), @food_id, CURDATE(), 'outcome', 45.50, 'Lunch at Italian restaurant', 'Great pasta'),
(UUID(), @transport_id, CURDATE(), 'outcome', 12.00, 'Uber to office', 'Morning commute'),
(UUID(), @salary_id, CURDATE(), 'income', 3500.00, 'Monthly salary', 'Regular income');

-- Yesterday
INSERT INTO cash_flows (id, category_id, belongs_date, flow_type, amount, description, remark) VALUES
(UUID(), @shopping_id, DATE_SUB(CURDATE(), INTERVAL 1 DAY), 'outcome', 89.99, 'New shoes', 'Sports shoes'),
(UUID(), @entertainment_id, DATE_SUB(CURDATE(), INTERVAL 1 DAY), 'outcome', 25.00, 'Movie tickets', 'Watched latest film');

-- This week (within last 7 days)
INSERT INTO cash_flows (id, category_id, belongs_date, flow_type, amount, description, remark) VALUES
(UUID(), @utilities_id, DATE_SUB(CURDATE(), INTERVAL 3 DAY), 'outcome', 150.00, 'Electricity bill', 'Monthly bill'),
(UUID(), @healthcare_id, DATE_SUB(CURDATE(), INTERVAL 4 DAY), 'outcome', 65.00, 'Pharmacy', 'Prescription refill'),
...
INSERT INTO cash_flows (id, category_id, belongs_date, flow_type, amount, description, remark) VALUES
(UUID(), @food_id, DATE_SUB(CURDATE(), INTERVAL 10 DAY), 'outcome', 120.00, 'Grocery shopping', 'Weekly groceries'),
(UUID(), @transport_id, DATE_SUB(CURDATE(), INTERVAL 12 DAY), 'outcome', 45.00, 'Gas station', 'Fill up tank'),
(UUID(), @food_id, DATE_SUB(CURDATE(), INTERVAL 15 DAY), 'outcome', 75.50, 'Dinner with friends', 'Birthday celebration'),
(UUID(), @entertainment_id, DATE_SUB(CURDATE(), INTERVAL 18 DAY), 'outcome', 30.00, 'Concert tickets', 'Live music event'),
(UUID(), @shopping_id, DATE_SUB(CURDATE(), INTERVAL 20 DAY), 'outcome', 250.00, 'Clothing', 'New wardrobe'),
(UUID(), @freelance_id, DATE_SUB(CURDATE(), INTERVAL 22 DAY), 'income', 500.00, 'Freelance project', 'Web design project'),
(UUID(), @utilities_id, DATE_SUB(CURDATE(), INTERVAL 25 DAY), 'outcome', 80.00, 'Internet bill', 'Monthly service');

-- Print summary
SELECT 
    'Demo data loaded successfully!' AS message,
    (SELECT COUNT(*) FROM cash_flows WHERE flow_type = 'income') AS total_income_transactions,
    (SELECT COUNT(*) FROM cash_flows WHERE flow_type = 'outcome') AS total_outcome_transactions,
    ...
    (SELECT SUM(amount) FROM cash_flows WHERE flow_type = 'outcome') AS total_outcome_amount,
