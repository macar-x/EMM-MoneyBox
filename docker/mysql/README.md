...
This directory contains MySQL initialization scripts for Cashlens. The scripts are separated into schema initialization and optional demo data to provide flexibility for different deployment scenarios.
...
```bash
mysql -u cashlens -p cashlens123 cashlens < docker/mysql/init-mysql-demo.sql
```
...
# Verify schema
docker exec -it cashlens-mysql mysql -u cashlens -p cashlens123 cashlens -e "SHOW TABLES;"
```

### Load Demo Data Manually
```bash
# Option 1: Direct SQL
docker exec -i cashlens-mysql mysql -u cashlens -p cashlens123 cashlens < docker/mysql/init-mysql-demo.sql

# Option 2: Via CLI (after building cashlens)
cd backend
go build -o cashlens main.go

# Create Excel from demo SQL (manual step) or use CLI:
./cashlens manage import -i path/to/demo-data.xlsx
```

### Query Examples
```bash
# View categories
docker exec -it cashlens-mysql mysql -u cashlens -p cashlens123 cashlens \
...
1. **Backup volumes** before production:
   ```bash
   docker run --rm -v cashlens_mysql_data:/data -v $(pwd):/backup ubuntu \
