...
This directory contains MongoDB initialization scripts for Cashlens. The scripts are separated into schema initialization and optional demo data to provide flexibility for different deployment scenarios.
...
```bash
docker exec -i cashlens-mongodb mongosh -u cashlens -p cashlens123 \
  --authenticationDatabase admin cashlens < docker/mongodb/init-mongo-demo.js
```
...
```bash
# View categories
docker exec -it cashlens-mongodb mongosh -u cashlens -p cashlens123 \
...
1. **Backup volumes** before production:
   ```bash
   docker run --rm -v cashlens_mongodb_data:/data -v $(pwd):/backup ubuntu \
