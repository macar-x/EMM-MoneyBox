# Cashlens API - Local Docker Deployment & Testing Guide

**Quick guide to deploy and test the Cashlens API locally**
...
# Expected output:
# NAME                 STATUS              PORTS
# cashlens-mongodb     Up X seconds        0.0.0.0:27017->27017/tcp
```
...
```bash
docker run -d \
  --name cashlens-mongodb \
  -p 27017:27017 \
  -e MONGO_INITDB_ROOT_USERNAME=cashlens \
  -e MONGO_INITDB_ROOT_PASSWORD=cashlens123 \
  -e MONGO_INITDB_DATABASE=cashlens \
  mongo:7.0
```
...
**Expected**: You should see a `cashlens` binary (size ~20-30 MB)
...
- Verify credentials: `cashlens:cashlens123`
...
```bash
# Start in background
nohup ./cashlens server start -p 8080 > server.log 2>&1 &
...
```bash
./cashlens server start -p 8081
```

Then test with: `curl http://localhost:8081/api/health`

### Problem: Permission denied when running ./cashlens
...
docker stop cashlens-mongodb
docker rm cashlens-mongodb
```
...
- [ ] Database connection works (`./cashlens db connect`)
...
3. **Import test data** - Use `./cashlens manage import`
4. **Export data** - Use `./cashlens manage export`
