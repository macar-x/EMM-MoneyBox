...
# Cashlens Docker Bootstrap Script
# This script helps you easily start the Cashlens application with Docker
...
start_services() {
    ...
    print_header "Starting Cashlens Services"
...
show_info() {
    ...
    if [[ "$ENABLE_SERVICES" == *"mongodb"* ]] || [[ "$1" == *"mongodb"* ]]; then
        ...
        local user=${MONGO_ROOT_USERNAME:-cashlens}
        local pass=${MONGO_ROOT_PASSWORD:-cashlens123}
        echo -e "MongoDB: ${GREEN}mongodb://$user:$pass@localhost:$port${NC}"
    fi

    if [[ "$ENABLE_SERVICES" == *"mysql"* ]] || [[ "$1" == *"mysql"* ]]; then
        local port=${MYSQL_PORT:-3306}
        local user=${MYSQL_USER:-cashlens}
        local pass=${MYSQL_PASSWORD:-cashlens123}
...
# Main function
main() {
    print_header "Cashlens Docker Bootstrap"
    ...
    print_header "Done!"
    print_success "Cashlens is ready to use!"
}
